...
function retrieveCoordinatesFromGeofencecode (geofenceCode: string): Array<Coordinate> {
...
function checkCoordinatesWithinBoundary(boundary:Array<Coordinate>, coordinates: Coordinate): boolean {
  const isCoordinatesWithinBoundary: boolean = geolib.isPointInPolygon(coordinates, boundary);
  return isCoordinatesWithinBoundary;
}

export async function isWithinGeofence(rideId:string, coordinates:Coordinate) {
  ...
  Boolean(validGeofenceCodes.length) && validGeofenceCodes.forEach((geofenceCode:string) => {
    const boundary: Array<Coordinate> = retrieveCoordinatesFromGeofencecode(geofenceCode)
